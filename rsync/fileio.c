...
static ssize_t write_sparse(int f, char *buf, int len)
{
	int l1 = 0, l2 = 0;
	ssize_t ret;
...
ssize_t flush_write_file(int f)
{
	ssize_t ret = 0;
	char *bp = wf_writeBuf;
 ...
 */
ssize_t write_file(int f, char *buf, int len)
{
	int ret = 0;
...
char *map_ptr(struct map_struct *map, OFF_T offset, int32 len)
{
	...
	while (read_size > 0) {
		ssize_t nread = read(map->fd, map->p + read_offset, read_size);
		if (nread <= 0) {
			if (!map->status)
