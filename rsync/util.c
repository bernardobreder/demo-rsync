...
static void glob_match(char *arg, int abpos, int fbpos)
{
	...
	while (*arg == '.' && arg[1] == '/') {
		if (fbpos < 0) {
			ENSURE_MEMSPACE(vglob.filt_buf, char, vglob.fbsize, vglob.absize);
			memcpy(vglob.filt_buf, vglob.arg_buf, abpos + 1);
			fbpos = abpos;
		}
		ENSURE_MEMSPACE(vglob.arg_buf, char, vglob.absize, abpos + 3);
		vglob.arg_buf[abpos++] = *arg++;
		vglob.arg_buf[abpos++] = *arg++;
		vglob.arg_buf[abpos] = '\0';
	}
	...
	if (strpbrk(arg, "*?[")) {
		...
		if (!(d = opendir(abpos ? vglob.arg_buf : ".")))
			return;
				   ...
				   char *arg, int abpos, int fbpos)
{
	...
	ENSURE_MEMSPACE(vglob.arg_buf, char, vglob.absize, abpos + len + 2);
	memcpy(vglob.arg_buf + abpos, name, len);
	abpos += len;
	vglob.arg_buf[abpos] = '\0';

	if (fbpos >= 0) {
		ENSURE_MEMSPACE(vglob.filt_buf, char, vglob.fbsize, fbpos + len + 2);
		memcpy(vglob.filt_buf + fbpos, name, len);
		fbpos += len;
		vglob.filt_buf[fbpos] = '\0';
		use_buf = vglob.filt_buf;
	} else
		use_buf = vglob.arg_buf;

	if (from_glob || (arg && len)) {
		STRUCT_STAT st;
		int is_dir;

		if (do_stat(vglob.arg_buf, &st) != 0)
			return;
	...
	if (arg) {
		...
		vglob.arg_buf[abpos] = '\0';
		if (fbpos >= 0) {
			vglob.filt_buf[fbpos++] = '/';
			vglob.filt_buf[fbpos] = '\0';
		}
		glob_match(arg, abpos, fbpos);
	} else {
		ENSURE_MEMSPACE(vglob.argv, char *, vglob.maxargs, vglob.argc + 1);
		if (!(vglob.argv[vglob.argc++] = strdup(vglob.arg_buf)))
...
int glob_expand(const char *arg, char ***argv_p, int *argc_p, int *maxargs_p)
{
	...
	ENSURE_MEMSPACE(vglob.arg_buf, char, vglob.absize, MAXPATHLEN);
	*vglob.arg_buf = '\0';

	vglob.argc = save_argc = *argc_p;
	vglob.argv = *argv_p;
	vglob.maxargs = *maxargs_p;

	ENSURE_MEMSPACE(vglob.argv, char *, vglob.maxargs, 100);

	glob_match(s, 0, -1);

	/* The arg didn't match anything, so add the failed arg to the list. */
	if (vglob.argc == save_argc) {
		ENSURE_MEMSPACE(vglob.argv, char *, vglob.maxargs, vglob.argc + 1);
		vglob.argv[vglob.argc++] = s;
		ret = 0;
	} else {
		free(s);
		ret = 1;
